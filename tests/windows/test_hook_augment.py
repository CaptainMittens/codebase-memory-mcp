...
def main():
    ...
    try:
        ...
        print("daemon start rc=%d stdout=%r stderr=%r" %
              (start.returncode, start_out[:120], start_err[:500]))
        ...
        if '"nodes"' not in idx_out:
            ...
            print("SETUP FAIL: index did not run (rc=%d):\nstdout=%s\nstderr=%s" %
                  (idx.returncode, idx_out[:300], idx_err[:2000]))
            return 2
