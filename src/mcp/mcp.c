...
char *cbm_mcp_render_search_rows_for_testing(const char *const *qualified_names,
    ...
    char *payload =
        render_search_payload(results, row_count, NULL, 0, row_count, 0, row_count, row_count, 0, 0,
                              ...
                              0, "", false, false, 0, false, json_format);
    free_search_results(results, initialized);
    return payload;
}
#endif
...
static char *handle_search_code(cbm_mcp_server_t *srv, const char *args) {
    ...
    char *payload = render_search_payload(
        ...
        raw_output, directory_start, directory_output, raw_limit, directory_limit, match_limit,
        mode, context_lines, source_max_lines, root_path, scan_saturated, warn_literal_pipe,
        elapsed_ms, false, sc_legacy_json);
...
static void adr_list_sections_from_content(yyjson_mut_doc *doc, yyjson_mut_val *root_obj,
    ...
    while (p && *p) {
        ...
        if (linelen > 0 && p[0] == '#') {
            char *header = cbm_strndup(p, linelen);
            if (header) {
                yyjson_mut_arr_add_strcpy(doc, sections, header);
                free(header);
            } else {
                yyjson_mut_arr_add_str(doc, sections, "(out-of-memory)");
            }
        }
        if (!eol) {
            break;
...
static char *handle_manage_adr(cbm_mcp_server_t *srv, const char *args) {
    ...
    bool valid_mode = strcmp(mode_str, "outline") == 0 || strcmp(mode_str, "get") == 0 ||
                      ...
                      strcmp(mode_str, "store") == 0;
    if (!valid_mode) {
        free(project);
        free(mode_str);
        free(content);
        return cbm_mcp_text_result("invalid mode: use outline, get, sections, or update", true);
    }
